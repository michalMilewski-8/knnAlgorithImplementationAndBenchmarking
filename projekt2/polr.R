...
get_csv_and_use_it_for_testing<- function(name){
  ...
  # zadbanie o liniwą niezależność kolumn
  X <- WeightIt::make_full_rank(X)
  ...
  for(i in 1:5){
    ...
    # zadbanie o liniwą niezależność kolumn (z jakiegoś powodu po podziale dla jednego ze zbiorów benchmarkowych wciąż pojawiały się kolumny liniowo zależne)
    subX <- data.frame(WeightIt::make_full_rank(as.matrix(subX)))
    
    subX$etykieta <- factor(suby, levels = 1:max(suby)) 
    subX <- subX[,c(ncol(subX),1:(ncol(subX)-1))]
    
    classifier <- MASS::polr(formula = formula(subX),data = subX,method = "logistic",Hess = TRUE)
...
data_sets <- c("abalone","abalone_ord","affairs","ailerons","auto_ord","auto_riskness","bostonhousing","bostonhousing_ord","cement_strength","fireman_example","glass","kinematics","stock_ord","winequality-red","winequality-white","wisconsin_breast_ord")
...
write.csv(bench_results,"other_benchmarks/polr_benchmark.csv")